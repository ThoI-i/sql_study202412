...
ON A.emp_no = B.emp_no -- 조인조건은 사번이 일치하는
JOIN tb_certi C  -- 4. 추가로 자격증 테이블 조인
ON B.certi_cd = C.certi_cd  -- 5. 조인 조건은 자격증번호
WHERE C.certi_nm = 'SQLD'; - -- 6. 추가로 자격증 명이 SQLD인 것 필터링
